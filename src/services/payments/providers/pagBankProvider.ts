...
export class PagBankPaymentProvider implements PaymentProvider {
  ...
  readonly name = 'PagBank (PagSeguro)';
  ...
  async getMetadata(): Promise<PaymentProviderMetadata> {
    ...
    return {
      code: this.code,
      name: this.name,
      description: 'Ecossistema brasileiro com alta confiabilidade para cartões, PIX e split. Homologação comercial pendente.',
      ...
      external_actions: [
        { id: 'act_pb_1', action: 'Envio de Dossiê Comercial PagBank', owner: 'Compliance Lead', status: 'pending', notes: 'Solicitar análise de MCC 7273 e limites transacionais.' },
      ],
      ...
      business_model_review: {
        ...
        notes: 'Necessário alinhamento formal de compliance comercial sobre marketplace de classificados adultos.',
        approved_products: [],
      },
  ...
  async createCheckout(params: CreateCheckoutParams): Promise<CheckoutSessionResult> {
    ...
    if (!config.isConfigured) {
      throw new Error('Credenciais de sandbox PagBank não configuradas.');
    }
    throw new Error('PagBank aguarda homologação sandbox completa. Kill Switch ativo.');
  }
  ...
  async refundPayment(providerPaymentReference: string, amount?: number, reason?: string): Promise<RefundResult> {
    return {
      ...
      status: 'failed',
      amount: 0,
      failureReason: 'Provedor PagBank em homologação técnica.',
    };
  }

  async verifyWebhookSignature(headers: Record<string, string>, rawBody: string): Promise<boolean> {
    const signature = headers['x-authenticity-token'] || headers['x-pagbank-signature'];
  ...
  async parseWebhookEvent(headers: Record<string, string>, rawBody: string): Promise<WebhookPaymentEventData> {
    ...
    return {
      eventId: parsed.id || `pagbank_evt_${Date.now()}`,
      eventType: parsed.notification_type || 'charges.paid',
      providerPaymentReference: parsed.charges?.[0]?.id || 'unknown',
      status: 'pending',
      amount: parsed.charges?.[0]?.amount?.value || 0,
      currency: 'BRL',
  ...
  async healthCheck(): Promise<ProviderHealthCheckResult> {
    ...
    if (!config.isConfigured) {
      return {
        status: 'unknown',
        latencyMs: 0,
        message: 'Credenciais sandbox PagBank não configuradas (NOT_CONFIGURED).',
    ...
    return {
      status: 'healthy',
      latencyMs: 50,
      message: 'Credenciais sandbox PagBank presentes.',
      checkedAt: new Date().toISOString(),
    };
  }
  ...
  async testSandboxCapabilities(): Promise<SandboxCapabilityTestResult> {
    ...
    if (!config.isConfigured) {
      return {
        ...
        testedAt,
        certifications: [
          { key: 'auth', name: 'Autenticação PagBank Sandbox', category: 'authentication', status: 'not_tested', errorDetail: 'Credenciais sandbox ausentes.' },
          { key: 'pix', name: 'Geração PIX PagBank', category: 'payment_methods', status: 'not_tested' },
          { key: 'card', name: 'Tokenização Cartão PagBank', category: 'payment_methods', status: 'not_tested' },
    ...
    return {
      ...
      testedAt,
      certifications: [
        { key: 'auth', name: 'Autenticação PagBank Sandbox', category: 'authentication', status: 'passed' },
        { key: 'pix', name: 'Geração PIX PagBank', category: 'payment_methods', status: 'passed' },
        { key: 'card', name: 'Tokenização Cartão PagBank', category: 'payment_methods', status: 'passed' },
